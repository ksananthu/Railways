...
import os
import pandas as pd
from py_files.fop_xlsx_fun import fop_xlsx_writer
    ...
    while True:
        ...
        if str(add_view) == 'a':
            while True:
                ...
                c.execute("select TOTAL from fop WHERE {} LIKE '%{}%'".format('Date', Date))
                ...
                c.execute("select FOP from CashBook WHERE {} LIKE '%{}%'".format('Date', Date))
                v_cash = c.fetchall()
                ...
                c.execute("INSERT INTO fop (Date, PLS, WT, FRE, DFC, ST, TOTAL) VALUES (?, ?,?, ?, ?, ?, ?)",
                          (Date, PLS, WT, FRE, DFC, ST, TOTAL))
                ...
                # #...............Edit function........................................................
                    ...
                    print('\n')
                    print('............Options.............')
                    print('No of Pls(PLS), Wt in Qtls(WT), DFC, ST, exit')
                    print('\n')

                    while True:
                        option_list = ['PLS', 'WT', 'ST', 'DFC', 'exit']
                        ...
                        if edit_in not in option_list:
                            ...
                            c.execute("UPDATE fop SET {} = '{}' WHERE Date = '{}'".format(edit_in, new_edit, Date))
                            conn.commit()
                            continue
                            ...
                            # >>>>>>> total >>>>>>>>>>>>>>>
                            c.execute("SELECT FRE, DFC, ST FROM fop WHERE {} = '{}' ".format('Date', Date))
                            ...
                            c.execute("UPDATE fop SET Total = '{}' WHERE Date = '{}'".format(n_total, Date))
                            conn.commit()
            ...
            df = pd.read_sql_query(
                "SELECT * FROM fop  ORDER BY Date ASC", conn)
            print(df)
            print('\n')
            ...
            while True:
                ...
                if dlt_opt == 'd':
                    ...
                    c.execute("DELETE FROM fop WHERE {} = '{}'".format('Date', dlt_date))
                    conn.commit()
