...
def lop_xlsx_writer():
    ...
    # # .....................creating xlsx file................................................................
    workbook = xlsxwriter.Workbook(str(dire) + '\\xlsx\\' + y + '\\lop.xlsx')
    worksheet2 = workbook.add_worksheet('lop')
    ...
    c.execute("select * from lop WHERE Date BETWEEN '{}' AND '{}' ORDER BY Date ASC".format(sort_date1, sort_date2))
    data1 = c.fetchall()

    # # Caption writing
    cap_m_y = str(y).replace('_', ' ').upper()
    caption = 'LOP return of CLT parcel for the month of  ' + cap_m_y
    ...
    worksheet2.set_column('A:H', 10)
    worksheet2.set_row(0, 30)
    worksheet2.set_default_row(16)

    worksheet2.merge_range('A1:H1', caption, merge_format)

    # Add a table to the worksheet.
    worksheet2.add_table('A2:H12', {'data': data1,
    ...
    c.execute("select * from lop WHERE Date BETWEEN '{}' AND '{}' ORDER BY Date ASC".format(sort_date10, sort_date20))
    data2 = c.fetchall()

    # Add a table to the worksheet.
    worksheet2.add_table('A14:H23', {'data': data2, 'header_row': False, 'style': 'Table Style Light 15'})
    ...
    c.execute("select * from lop WHERE Date BETWEEN '{}' AND '{}' ORDER BY Date ASC".format(sort_date100, sort_date200))
    data3 = c.fetchall()

    # Add a table to the worksheet.
    worksheet2.add_table('A25:H35', {'data': data3, 'header_row': False, 'style': 'Table Style Light 15'})
    ...
    c.execute("select TOTAL from lop ORDER BY Date ASC")
    amount = c.fetchall()
    total_amt = 0
    # print(amount)
    for j in amount:
    ...
    worksheet2.write('H41', 'CPS/CLT', cell_format)

    workbook.close()

    # # opening the file
    webbrowser.open(str(dire) + '\\xlsx\\' + y + '\\lop.xlsx')
    print('\n')
