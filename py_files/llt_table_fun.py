...
import os
import pandas as pd
from py_files.llt_xlsx_fun import llt_xlsx_writer
    ...
    while True:
        ...
        if str(add_view) == 'a':
            while True:
                ...
                c.execute("select TOTAL from llt WHERE {} LIKE '%{}%'".format('Date', Date))
                ...
                c.execute("select LLT from CashBook WHERE {} LIKE '%{}%'".format('Date', Date))
                v_cash = c.fetchall()
                ...
                c.execute("INSERT INTO llt (Date, NL, WT, FRE, KFC, DFC, ST, TOTAL) VALUES (?, ?,?, ?, ?, ?, ?, ?)",
                          (Date, NL, WT, FRE, KFC, DFC, ST, TOTAL))
                ...
                # #...............Edit function........................................................
                    ...
                    print('\n')
                    print('............Options.............')
                    print('No of Lug(NL), Wt in Qtls(WT), KFC, DFC, ST, exit')
                    print('\n')

                    while True:
                        option_list = ['NL', 'WT', 'ST', 'KFC', 'DFC', 'exit']
                        ...
                        if edit_in not in option_list:
                            ...
                            c.execute("UPDATE llt SET {} = '{}' WHERE Date = '{}'".format(edit_in, new_edit, Date))
                            conn.commit()
                            continue
                            ...
                            # >>>>>>> total >>>>>>>>>>>>>>>
                            c.execute("SELECT FRE, KFC, DFC, ST FROM llt WHERE {} = '{}' ".format('Date', Date))
                            ...
                            c.execute("UPDATE llt SET Total = '{}' WHERE Date = '{}'".format(n_total, Date))
                            conn.commit()
            ...
            df = pd.read_sql_query(
                "SELECT * FROM llt  ORDER BY Date ASC", conn)
            print(df)
            print('\n')
            ...
            while True:
                ...
                if dlt_opt == 'd':
                    ...
                    c.execute("DELETE FROM LLT WHERE {} = '{}'".format('Date', dlt_date))
                    conn.commit()
